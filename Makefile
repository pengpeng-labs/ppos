...
.PHONY: all check component ppnet-component run run-headless test test-smoke test-boundary test-elf verify clean

all: $(BUILD)/ppos-v0.2.0.elf
...
$(BUILD)/ppos-kernel.elf64: $(TARGET_OBJECT) component ppnet-component | $(BUILD)
	$(LD) $(LD_FLAGS) -T $(OSBARE_DIR)/arch/x86_64/kernel64.ld \
		...
		$(PPNET_DIR)/build/third_party/libppnet-c.a \
...
$(BUILD)/ppos-v0.2.0.elf: $(BUILD)/ppos-kernel.bin.o component
